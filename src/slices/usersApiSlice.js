...
export const usersApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getProfileDetails: builder.query({ query: () => ({ url: `${USERS_URL}/profile` }), providesTags: ['User'] }),
    updateProfile: builder.mutation({ query: (data) => ({ url: `${USERS_URL}/profile`, method: 'PUT', body: data }), invalidatesTags: ['User'] }),
  }),
});

export const {
  useLoginMutation, useLogoutMutation, useRegisterMutation,
  useUpdateProfileMutation, useGetProfileDetailsQuery,
} = usersApiSlice;