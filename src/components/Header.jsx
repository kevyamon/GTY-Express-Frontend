...
const Header = () => {
  ...
  // ----------------------------------------------------------------
  // 5. Calculs mémoïsés pour les performances
  // ----------------------------------------------------------------
  const { newOrdersCount, cancelledOrdersCount, unreadNotifsCount } = useMemo(() => {
    const lastSeen = new Date(lastSeenAdminTimestamp);
    const newOrders = userInfo?.isAdmin && Array.isArray(adminOrders) ? adminOrders.filter(o => new Date(o.createdAt) > lastSeen).length : 0;
    const cancelledOrders = userInfo?.isAdmin && Array.isArray(adminOrders) ? adminOrders.filter(o => o.status === 'Annulée' && new Date(o.updatedAt) > lastSeen).length : 0;
    const unreadNotifs = userInfo && Array.isArray(notifications) ? notifications.filter(n => !n.isRead).length : 0;
