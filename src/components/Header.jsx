...
const Header = () => {
  ...
  const handleNotificationClick = async () => {
    if (unreadNotifsCount > 0) {
      await markAsRead().unwrap().catch(err => console.error('Erreur:', err));
    }
  ...
  return (
    <header className="header-layout">
                  ...
                  >
                    ...
                    {userInfo.isAdmin && (
                      <>
                        ...
                        <NavDropdown.Item as={Link} to="/admin/orderlist" onClick={handleAdminMenuClick}>Gestion Commandes {newOrdersCount > 0 && (<Badge pill bg="primary" className="ms-2">{newOrdersCount}</Badge>)} {cancelledOrdersCount > 0 && (<Badge pill bg="warning" text="dark" className="ms-2">{cancelledOrdersCount}</Badge>)}</NavDropdown.Item>
                        <NavDropdown.Item as={Link} to="/admin/promotionlist">Gestion Promotions</NavDropdown.Item>
                      </>
                    )}
                    <NavDropdown.Divider />
      ...
      <div className="header-center-row bg-dark">
        ...
        {userInfo && (
          <div className="header-icon-group mt-3">
            ...
            <Link to="/notifications" onClick={handleNotificationClick} className="home-icon-link">
              <span style={{ position: 'relative' }}>
                🔔
            ...
            <Link to="/supermarket" className="supermarket-btn">
