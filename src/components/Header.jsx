...
import { useState, useMemo, useEffect } from 'react'; // Importer useEffect
...
const Header = () => {
  ...
  // --- Logique pour les notifications Admin & Client ---
  ...
  // --- NOUVELLE LOGIQUE POUR LE COMPTEUR ADMIN ---
  const [lastSeenOrderCount, setLastSeenOrderCount] = useState(
    () => Number(localStorage.getItem('lastSeenOrderCount')) || 0
  );
  ...
  const { newOrdersCount, hasNotification } = useMemo(() => {
    ...
    if (userInfo?.isAdmin && adminOrders) {
      newOrders = adminOrders.length - lastSeenOrderCount;
      if (newOrders < 0) newOrders = 0; // Au cas où des commandes sont supprimées
    } else if (userInfo && !userInfo.isAdmin && clientOrders) {
      notification = clientOrders.some((o) => o.status === 'Confirmée' || o.status === 'Expédiée');
    }
    return { newOrdersCount: newOrders, hasNotification: notification };
  }, [userInfo, adminOrders, clientOrders, lastSeenOrderCount]);

  const handleAdminMenuClick = () => {
    if (adminOrders) {
      const currentOrderCount = adminOrders.length;
      localStorage.setItem('lastSeenOrderCount', currentOrderCount);
  ...
  const submitHandler = (e) => { e.preventDefault(); if (keyword.trim()) { navigate(`/search/${keyword}`); setKeyword(''); } else { navigate('/products'); } };
  ...
  return (
    ...
    <div className="header-center-row bg-dark">
      <Form onSubmit={submitHandler} className="d-flex search-form">{/* ... */}</Form>
